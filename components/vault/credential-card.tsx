...
export function CredentialCard({ credential, onEdit, onDelete }: CredentialCardProps) {
    ...
    return (
        <div className="bg-background border border-border hover:border-foreground/20 p-5 transition-all group flex flex-col justify-between h-full relative overflow-hidden">
            {/* Background pattern */}
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-foreground/5 border border-foreground/10 flex items-center justify-center text-foreground font-mono font-bold text-lg group-hover:border-foreground/30 transition-colors">
                            {credential.decrypted.site_name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                            <h3 className="text-foreground font-bold leading-tight line-clamp-1 tracking-wide">{credential.decrypted.site_name}</h3>
                            <span className="mono text-[10px] text-fg-muted uppercase tracking-widest">{credential.category}</span>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="p-1.5 text-fg-muted hover:text-foreground transition-colors"
                        >
                        ...
                        {showMenu && (
                            <>
                                ...
                                />
                                <div className="absolute right-0 top-full mt-2 w-36 bg-background border border-border shadow-2xl z-20 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                                    <button
                                        onClick={() => { setShowMenu(false); onEdit(credential); }}
                                        className="w-full text-left px-4 py-3 text-xs mono uppercase tracking-widest text-fg-secondary hover:bg-foreground/5 hover:text-foreground flex items-center gap-3 transition-colors"
                                    >
                                        <Edit2 className="w-3.5 h-3.5" /> Edit
                                    </button>
                                    <button
                                        onClick={() => { setShowMenu(false); onDelete(credential.id); }}
                                        className="w-full text-left px-4 py-3 text-xs mono uppercase tracking-widest text-red-500/80 hover:bg-red-500/10 hover:text-red-500 flex items-center gap-3 transition-colors border-t border-border"
                                    >
                ...
                {credential.category === "secure_note" ? (
                    <div className="space-y-2 mb-4 flex-1">
                        <div className="text-sm text-fg-secondary bg-foreground/5 p-3 h-full max-h-32 overflow-hidden relative border border-foreground/5">
                    ...
                    <div className="space-y-4 mb-4">
                        <div className="flex items-center justify-between text-sm group/item">
                            <span className="text-fg-secondary truncate mr-2 tracking-wide text-xs" title={credential.decrypted.username}>
                        ...
                        <div className="flex items-center justify-between text-sm group/item">
                            <div className="text-fg-muted font-mono tracking-widest flex items-center gap-3 text-xs">
            ...
            {/* Bottom Metadata */}
            <div className="mt-6 pt-4 border-t border-border flex justify-between items-center text-[10px] mono text-fg-muted uppercase tracking-widest relative z-10">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {credential.is_favorite && <span className="text-foreground">★</span>}
                </div>
            </div>
