// @ts-nocheck — ported WebGPU scene; types deferred for maintainability.
...
/** Mount the WebGPU voxel shield scene into `host`. Returns dispose. */
export async function mountVaultHeroScene(host: HTMLElement): Promise<() => void> {
    let ao, denoise, ssrModule, bloomModule;
    try {
        const gtaoMod = await import("three/addons/tsl/display/GTAONode.js");
        ao = gtaoMod.ao;
    } catch (e: unknown) {
        console.warn("GTAO not available:", e instanceof Error ? e.message : e);
    }
    try {
        const denoiseMod = await import("three/addons/tsl/display/DenoiseNode.js");
        denoise = denoiseMod.denoise;
    } catch (e: unknown) {
        console.warn("Denoise not available:", e instanceof Error ? e.message : e);
    }
    try {
        ssrModule = await import("three/addons/tsl/display/SSRNode.js");
    } catch (e: unknown) {
        console.warn("SSR not available:", e instanceof Error ? e.message : e);
    }
    try {
        bloomModule = await import("three/addons/tsl/display/BloomNode.js");
    } catch (e: unknown) {
        console.warn("Bloom not available:", e instanceof Error ? e.message : e);
    }

    const sceneContainer = host;
    Object.assign(host.style, { width: "100%", height: "100%", display: "block" });
...
try {
    ...
    if (ssrModule && ssrModule.ssr) {
        try {
            ...
            (window as unknown as { _ssrStrength?: unknown })._ssrStrength = ssrStrengthUniform;
